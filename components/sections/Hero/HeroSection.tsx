...
const HeroSection = () => {
  return (
    <section className="relative flex flex-col md:flex-row items-center justify-center px-6 md:px-20 pt-20 w-full min-h-screen overflow-hidden">
      {/* Contenido del Hero */}
      <motion.div
        initial="hidden"
        animate="visible"
        className="flex flex-col md:flex-row items-center justify-center w-full z-20"
      >
        {/* Texto izquierda */}
        <div className="w-full flex flex-col gap-5 justify-center text-start">
          {/* Bienvenida */}
          <motion.div variants={slideInFromTop(0.5)}>
            <div className="py-2 px-3 border border-[#7042f88b] opacity-90 flex items-center Welcome-box bg-black/30 backdrop-blur-sm rounded-md">
          ...
          <motion.div
            ...
            className="flex flex-col gap-2 mt-6 text-white max-w-[600px] leading-tight"
          >
            <Title level="h1">
              Ofreciendo{' '}
          ...
          <motion.div variants={slideInFromLeft(0.8)}>
            <Paragraph className="text-gray-400 my-5 max-w-[600px] text-lg">
              Bienvenido a mi laboratorio digital, donde experimento con ideas, construyo soluciones y aprendo algo nuevo cada día. 
              Disfruto resolver problemas con tecnología y transformar ideas en soluciones funcionales. 
